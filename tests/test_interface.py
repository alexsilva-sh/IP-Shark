...
def linha_ip(indice, ip, status, abuse, vt, ibm, pais):
    colunas = (ip, gui.t(gui.VERDICT_KEYS[status]), abuse, vt, ibm, pais)
...
check(gui.t("detail_hint") in app.tabela_ip.detalhe.get("1.0", tk.END), "painel mostra a dica inicial")

print("\n[2] Contadores ao vivo da entrada")
check(gui.IPCheckerApp._contar_ips("8.8.8.8 1.1.1.1") == "2 válidos", "conta so validos")
resumo = gui.IPCheckerApp._contar_ips("8.8.8.8, abc, 10.0.0.1")
check("1 válidos" in resumo and "1 inválidos" in resumo and "1 privados" in resumo,
      f"separa valido/invalido/privado ({resumo})")
check(gui.IPCheckerApp._contar_ips("") == "", "entrada vazia nao mostra contador")
check(gui.IPCheckerApp._contar_hashes("d41d8cd98f00b204e9800998ecf8427e xyz") == "1 válidos · 1 inválidos",
      "contador de hash separa invalidos")
check(gui.IPCheckerApp._contar_dominios("a.com b.com") == "2 itens", "contador de dominios")
...
check(app.tabela_ip.tree.set("ip-1", "veredito") == gui.t("verdict_clean"), "● Limpo")
check(app.tabela_ip.tree.set("ip-2", "veredito") == gui.t("verdict_bad"), "✖ Malicioso")
check(app.tabela_ip.tree.set("ip-3", "veredito") == gui.t("verdict_review"), "▲ Revisar")
...
check(gui.t("skipped_items") in relatorio, "relatorio lista os ignorados")
...
check(app.tabela_ip.tree.set("ip-erro-9", "veredito") == gui.t("verdict_unknown"), "erro marcado indisponivel")

print("\n[10] IPs associados como filhos do dominio")
app.tabela_url.clear()
app._linha_url(1, ("exemplo.com", gui.t("verdict_bad"), "-", 3, "8.0", "2"), "exemplo.com\ndetalhe", "bad")
app._linha_url_filha(1, 1, ("9.9.9.9", gui.t("verdict_clean"), "0%", 0, "1.0", "-"), "9.9.9.9\ndetalhe", "clean")
...
pedidos = []
core.filedialog.askdirectory = lambda **kw: pedidos.append(kw) or "C:\\saida"
raizes_antes = len(root.children)
app.results_ip = [["1.1.1.1", "0%", 0]]
app.ibm_ip_ativo = False
gravado = []
core.save_to_csv = lambda *a, **kw: gravado.append((a, kw)) or core.escolher_diretorio(
    kw.get("parent"), kw.get("titulo"))
gui.save_to_csv = core.save_to_csv
app.save_results()
check(len(pedidos) == 1, "o dialogo de pasta abriu")
check(pedidos[0].get("parent") is root, "dialogo usa a janela existente como dona")
check(pedidos[0].get("title") == gui.t("select_folder"), "titulo traduzido, sem citar CSV")
check(tk._default_root is root, "nenhuma segunda raiz Tk virou a padrao")

print("\n[15] Verificacao de versao nao segura a janela")
import threading
core_gui_versao = gui.check_latest_version
liberado = threading.Event()
gui.check_latest_version = lambda: (liberado.wait(5), (None, None))[1]
inicio = time.time()
t_versao = threading.Thread(target=gui.verificar_atualizacao_em_segundo_plano,
...
gui.check_latest_version = core_gui_versao
